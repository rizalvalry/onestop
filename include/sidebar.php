<!-- Sidebar -->
<ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">

<!-- Sidebar - Brand -->
<a class="sidebar-brand d-flex align-items-center justify-content-center" href="index.html">
    <div class="sidebar-brand-icon rotate-n-15">
        <i class="fas fa-laugh-wink"></i>
    </div>
    <div class="sidebar-brand-text mx-3">SB Admin <sup>2</sup></div>
</a>

<!-- Divider -->
<hr class="sidebar-divider my-0">

<!-- Nav Item - Dashboard -->
<li class="nav-item <?php echo (basename($_SERVER['PHP_SELF'])=="index1.php") ? "active" : "" ?>">
    <a class="nav-link" href="<?php echo getConfig('base_url');?>index1">
        <i class="fas fa-fw fa-tachometer-alt"></i>
        <span>Dashboard</span></a>
</li>

<!-- Divider -->
<hr class="sidebar-divider">



<!-- Heading -->
<div class="sidebar-heading">
Interface
</div>

<!-- Nav Item - Charts -->
<li class="nav-item <?php echo (basename($_SERVER['PHP_SELF'])=="tambahdatatransaksi.php") ? "active" : "" ?>">
    <a class="nav-link" href="<?php echo getConfig('base_url');?>tambahdatatransaksi">
        <i class="fas fa-fw fa-chart-area"></i>
        <span>Transaksi Laundry</span></a>
</li>

<!-- Nav Item - Tables -->
<li class="nav-item <?php echo (basename($_SERVER['PHP_SELF'])=="transaksi.php") ? "active" : "" ?>">
    <a class="nav-link" href="<?php echo getConfig('base_url');?>transaksi">
        <i class="fas fa-fw fa-chart-area"></i>
        <span>Data Transaksi</span></a>
</li>

<!-- Nav Item - Tables -->
<li class="nav-item <?php echo (basename($_SERVER['PHP_SELF'])=="pakaian.php") ? "active" : "" ?>">
    <a class="nav-link" href="<?php echo getConfig('base_url');?>pakaian">
        <i class="fas fa-fw fa-chart-area"></i>
        <span>Data Pakaian</span></a>
</li>

<!-- Nav Item - Tables -->
<li class="nav-item <?php echo (basename($_SERVER['PHP_SELF'])=="pelanggan.php") ? "active" : "" ?>">
    <a class="nav-link" href="<?php echo getConfig('base_url');?>pelanggan">
        <i class="fas fa-fw fa-chart-area"></i>
        <span>Data Pelanggan</span></a>
</li>

<!-- Divider -->
<hr class="sidebar-divider d-none d-md-block">

<!-- Sidebar Toggler (Sidebar) -->
<div class="text-center d-none d-md-inline">
    <button class="rounded-circle border-0" id="sidebarToggle"></button>
</div>

<!-- Sidebar Message -->
<div class="sidebar-card d-none d-lg-flex">
    <img class="sidebar-card-illustration mb-2" src="img/undraw_rocket.svg" alt="...">
    <p class="text-center mb-2"><strong>SB Admin Pro</strong> is packed with premium features, components, and more!</p>
    <a class="btn btn-success btn-sm" href="https://startbootstrap.com/theme/sb-admin-pro">Upgrade to Pro!</a>
</div>

</ul>
<!-- End of Sidebar -->