...
<div class="table-responsive">
  <table id="table-dryclean" class="table table-striped table-bordered" >
    <thead>
      <tr>
    ...
    <tbody>
      <?php
        ...
        where No_Order = $order AND d.Id_Laundry = 3 AND admin_id=$admin_id)
        as t1
        join (SELECT total_harga, Id_Pakaian as idpakaian FROM harga where no_order = $order AND Id_Laundry = 3) as t2 on t1.Id_Pakaian = t2.idpakaian");
        while ($hasil = mysqli_fetch_array($sql)) {
      ?>
      <tr>
          ...
          <button data-id="<?php echo $hasil['No_Order']; ?>" data-ids="<?php echo $hasil['Id_Pakaian']; ?>" data-idl="<?php echo $hasil['Id_Laundry']; ?>" class="btn btn-danger btn-sm hapus_data_dry"> <i class="fa fa-trash"></i> Hapus </button>
          </td>
      </tr>
...
<script type="text/javascript">
    ...
    $(document).on('click', '.hapus_data_dry', function(){
