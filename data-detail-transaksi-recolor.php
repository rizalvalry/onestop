<div class="form-group">
  ...
  $sql = mysqli_query($conn, "SELECT sum(total_berat) as total_berat, sum(total_harga) as total_harga FROM harga  where no_order = '$order' AND Id_Laundry = 5 AND admin_id=$admin_id");
  $hasil = mysqli_fetch_array($sql);
  ?>
...
<div class="table-responsive">
  <table id="table-recolor" class="table table-striped table-bordered" >
    <thead>
      <tr>
    ...
    <tbody>
      ...
      include "./include/koneksi.php";
        ...
        $sql = mysqli_query($conn, "SELECT p.Jenis_Pakaian, l.Jenis_Laundry, dt.No_Order, dt.Id_Pakaian, dt.Jumlah_pakaian, dt.Id_Laundry, h.total_harga FROM detail_transaksi dt
                                      ...
                                      Where dt.No_Order = $no_o AND h.Id_Laundry = 5 AND l.Jenis_Laundry = 'Recolour' AND dt.admin_id=$admin_id");
        while ($hasil = mysqli_fetch_array($sql)) {
      ...
      <tr>
        ...
        <button data-id="<?php echo $hasil['No_Order']; ?>" data-ids="<?php echo $hasil['Id_Pakaian']; ?>" data-idl="<?php echo $hasil['Id_Laundry']; ?>" class="btn btn-danger btn-sm hapus_data_rec"> <i class="fa fa-trash"></i> Hapus </button>
        </td>
      </tr>
  ...
  <div class="col-xs-2">
    ...
    <input type="text" name="hargarecolor" id="hargarecolor" value="<?= $hargarecolor['jumlah']; ?>" class="form-control" readonly="true">
  </div> -->


<script type="text/javascript">
    $(document).ready(function() {
        $('#table-recolor').DataTable();
    } );
    ...
    $(document).on('click', '.hapus_data_rec', function(){
